...
void Otto::putAnimationMouth(unsigned long int aniMouth, int index)
{
  ...
  led_mouth.clear();
  unsigned long temp = PROGMEM_getAnything(&Gesturetable[aniMouth][index]);
  uint8_t bitmap[8] = {(uint8_t)((temp >> 24)&0x3f), (uint8_t)((temp >> 18)&0x3f), (uint8_t)((temp >> 12)&0x3f), (uint8_t)((temp >> 6)&0x3f), (uint8_t)((temp >> 0)&0x3f)};
  led_mouth.drawBitmap(3, 2, bitmap, 8, 5, 1);
  led_mouth.writeDisplay();
}
...
void Otto::putMouth(unsigned long int mouth, bool predefined)
{
  ...
  if (predefined)
  {
    ...
    unsigned long temp = PROGMEM_getAnything(&Mouthtable[mouth]);
    uint8_t bitmap[8] = {(uint8_t)((temp >> 24)&0x3f), (uint8_t)((temp >> 18)&0x3f), (uint8_t)((temp >> 12)&0x3f), (uint8_t)((temp >> 6)&0x3f), (uint8_t)((temp >> 0)&0x3f)};
    led_mouth.drawBitmap(3, 2, bitmap, 8, 5, 1);
  }
  else
  {
    ...
    uint8_t bitmap[8] = {(uint8_t)((mouth >> 24)&0x3f), (uint8_t)((mouth >> 18)&0x3f), (uint8_t)((mouth >> 12)&0x3f), (uint8_t)((mouth >> 6)&0x3f), (uint8_t)((mouth >> 0)&0x3f)};
    led_mouth.drawBitmap(3, 2, bitmap, 8, 5, 1);
  }
